...
PROC EXPORT
===========
Permite exportar los datos de SAS en otros formatos.

Sintaxis:
	PROC EXPORT DATA = <data set> OUTFILE = <'archivo de salida'>
		[DBMS = <driver>] [REPLACE];
		[DELIMITER = <'caracter separador'>];
*/

PROC EXPORT DATA = helados OUTFILE = 'helados1.txt' REPLACE;
	DELIMITER = '|';
RUN;
